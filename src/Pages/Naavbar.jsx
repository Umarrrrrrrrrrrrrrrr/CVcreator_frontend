        ...
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex md:items-center md:gap-1">
            {projectLinks.map((link) => (
              <button key={link.path} onClick={() => handleNav(link.path)} className="px-4 py-2 text-gray-700 font-medium rounded-lg hover:bg-blue-50 hover:text-blue-600 transition-all duration-200">{link.label}</button>
            ))}
            {appLinks.map((link) => (
              <button
                key={link.path}
                ...
                className={`px-4 py-2 rounded-lg transition-all duration-200 flex items-center gap-1.5 ${canAccessApp ? "text-gray-700 font-medium hover:bg-blue-50 hover:text-blue-600" : "text-gray-400 cursor-pointer hover:bg-gray-100 hover:text-gray-500"}`}
              >
                {!canAccessApp && <svg className="w-4 h-4 flex-shrink-0" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M5 9V7a5 5 0 0110 0v2a2 2 0 012 2v5a2 2 0 01-2 2H5a2 2 0 01-2-2v-5a2 2 0 012-2zm8-2v2H7V7a3 3 0 016 0z" clipRule="evenodd" /></svg>}
                {link.label}
              </button>
            ))}
          </div>
          ...
          <div className="hidden md:block relative">
            ...
            {accountOpen && (
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-2xl border border-gray-100 py-2 z-50" onMouseLeave={() => setAccountOpen(false)}>
                {canAccessApp ? (
                  <>
          ...
          <div className="md:hidden">
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="inline-flex items-center justify-center p-2 rounded-md text-gray-700 hover:text-blue-600 hover:bg-blue-50 focus:outline-none">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden border-t border-gray-200">
          <div className="px-2 pt-2 pb-3 space-y-1">
            ...
            {appLinks.map((link) => (
              <button key={link.path} onClick={() => handleNav(link.path)} className={`block w-full text-left px-3 py-2 rounded-md font-medium flex items-center gap-2 ${canAccessApp ? "text-gray-700 hover:bg-blue-50 hover:text-blue-600" : "text-gray-400 hover:bg-gray-100"}`}>
